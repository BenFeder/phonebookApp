    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500">
          No contacts found. {isAuthenticated && <Link to="/add-contact" className="text-indigo-600 hover:underline">Add one now</Link>}
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {contacts.map((contact) => (
            <div key={contact._id} className="bg-white rounded-lg shadow p-6 hover:shadow-lg transition-shadow">
              <div className="flex justify-between items-start mb-3">
                ...
                {isAuthenticated && (
                  <button
                    ...
                    className={`text-2xl ${isFavorite(contact._id) ? 'text-yellow-500' : 'text-gray-300 hover:text-yellow-500'}`}
                    title={isFavorite(contact._id) ? 'Remove from favorites' : 'Add to favorites'}
                  >
                    {isFavorite(contact._id) ? '★' : '+'}
                  </button>
                )}
              ...
              {isAuthenticated && contact.createdBy._id === localStorage.getItem('userId') && (
